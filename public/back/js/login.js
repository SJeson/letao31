$(function () {
  ...
  // 这里存在一个问题：当点击登录按钮的时候，页面会进行跳转，但是同时用户信息也会拼接在浏览器的地址栏上面。需要阻止按钮的跳转事件。如果将这个按钮换成普通按钮，校验就会消失，但是用submit的话，点击之后会进行跳转，需要注册表单验证成功事件。表单校验成功时，会触发success.form.bv事件，此时会提交表单，这时候，通常我们需要禁止表单的自动提交，使用ajax进行表单的提交。
